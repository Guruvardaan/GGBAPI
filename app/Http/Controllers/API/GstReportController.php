...
class GstReportController extends Controller
{
    ...
    public function purchase_order_artical_wise()
    {
        ...
        if($type === 'default') {
            ...
            $total = sizeof($b2b_purchase_invoice) + sizeof($nil_reted_invoice);
        ...
        if($type === 'import_of_services_invoices') {
            $data = [];
            $total = 0;
        }

        if($type === 'itc_reversal') {
            $data = [];
            $total = 0;
        }

        if($type === 'tax_paid_on_reverse_charges') {
            $data = [];
            $total = 0;
        }

        if($type === 'tax_paid_under_reverse_charge_on_advance') {
            $data = [];
            $total = 0;
        }
